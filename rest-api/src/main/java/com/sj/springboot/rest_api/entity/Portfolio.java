// src/main/java/com/portfolio/entity/Portfolio.java
...
public class Portfolio {
    ...
    public BigDecimal getGainLossPercentage(BigDecimal currentPrice) {
        ...
        BigDecimal gainLoss = getGainLoss(currentPrice);
        return gainLoss.divide(totalInvestment, 4, BigDecimal.ROUND_HALF_UP).multiply(BigDecimal.valueOf(100));
    }
    ...
    public void addShares(Integer additionalShares, BigDecimal purchasePrice) {
        ...
        this.priceOfBuying = newTotalCost.divide(BigDecimal.valueOf(this.volume), 2, BigDecimal.ROUND_HALF_UP);
    ...
    public BigDecimal getDividendYield(BigDecimal annualDividendPerShare) {
        if (priceOfBuying.compareTo(BigDecimal.ZERO) == 0) {
            return BigDecimal.ZERO;
        }
        return annualDividendPerShare.divide(priceOfBuying, 4, BigDecimal.ROUND_HALF_UP).multiply(BigDecimal.valueOf(100));
    }
    
    public boolean isLongTermHolding(int months) {
        LocalDateTime cutoffDate = LocalDateTime.now().minusMonths(months);
        return timestamp.isBefore(cutoffDate);
    }
    
    public int getDaysHeld() {
        return (int) java.time.temporal.ChronoUnit.DAYS.between(timestamp.toLocalDate(), LocalDateTime.now().toLocalDate());
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(id, portfolio.id) &&
               Objects.equals(stockTicker, portfolio.stockTicker) &&
               Objects.equals(volume, portfolio.volume) &&
               Objects.equals(priceOfBuying, portfolio.priceOfBuying) &&
               Objects.equals(timestamp, portfolio.timestamp);
    }
